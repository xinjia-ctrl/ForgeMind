...
- [x] 生成有效的 Git commit；
- [ ] 全流程事件可逐步回放，且结果可复现（同输入同结果）。

> **实现状态说明（v0.2）**：前四项已由 e2e 测试（`tests/e2e/full-workflow.test.ts`，真实运行 `node --test` 并创建真实 Git commit）验证通过。第五项"结果可复现"以"**同输入 → 同流程、同事件序列、门禁判定可复现**"为达成口径（固定模型 + `temperature=0` + `seed=42`，尽力而为），非比特级复现。
