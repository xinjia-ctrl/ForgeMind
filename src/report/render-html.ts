...
function renderQuality(quality: ReportQuality): string {
  const coverage =
    quality.codeCoveragePercent === null
      ? "Unavailable"
      : `${quality.codeCoveragePercent.toFixed(2)}%`;
  const recommendations =
    quality.recommendations.length === 0
      ...
      : quality.recommendations
          ...
          .join("");
  return `<article class="quality-panel ${cssToken(quality.grade)}">
    <div class="quality-score"><span>${escapeHtml(quality.grade)}</span><strong>${formatNumber(quality.score)}</strong><small>/ 100</small></div>
    <dl class="quality-metrics">
      <div><dt>Gate pass rate</dt><dd>${quality.gatePassRate.toFixed(2)}% · ${formatNumber(quality.gatesPassed)}/${formatNumber(quality.gatesTotal)}</dd></div>
      <div><dt>Rework rounds</dt><dd>${formatNumber(quality.reworkRounds)}</dd></div>
      <div><dt>Test pass rate</dt><dd>${quality.testPassRate.toFixed(2)}% · ${formatNumber(quality.testsPassed)}/${formatNumber(quality.testsTotal)}</dd></div>
      <div><dt>Code coverage</dt><dd>${escapeHtml(coverage)} · ${escapeHtml(quality.coverageSource)}</dd></div>
    </dl>
    <div class="quality-recommendations"><strong>Evidence-driven recommendations</strong><ul>${recommendations}</ul></div>
  </article>`;
}
...
.quality-panel{display:grid;grid-template-columns:150px minmax(260px,1fr) minmax(280px,1.2fr);gap:24px;align-items:stretch;margin-bottom:64px;padding:22px;border:1px solid var(--line);border-left:4px solid var(--amber);border-radius:14px;background:linear-gradient(145deg,var(--panel2),var(--panel))}.quality-panel.excellent,.quality-panel.good{border-left-color:var(--green)}.quality-panel.poor{border-left-color:var(--red)}.quality-score{display:flex;flex-wrap:wrap;align-content:center;align-items:baseline;gap:5px;padding-right:20px;border-right:1px solid var(--line)}.quality-score span{flex-basis:100%;color:var(--amber);font:800 11px ui-monospace,monospace;letter-spacing:.08em}.quality-panel.excellent .quality-score span,.quality-panel.good .quality-score span{color:var(--green)}.quality-panel.poor .quality-score span{color:var(--red)}.quality-score strong{font:800 52px ui-monospace,monospace;line-height:1}.quality-score small{color:var(--muted)}.quality-metrics{margin:0}.quality-metrics div{padding:9px 0}.quality-recommendations{padding-left:20px;border-left:1px solid var(--line)}.quality-recommendations>strong{font-size:13px}.quality-recommendations ul{margin:12px 0 0;padding-left:18px;color:var(--muted);font-size:12px;line-height:1.6}.quality-recommendations li+li{margin-top:6px}@media(max-width:900px){.quality-panel{grid-template-columns:1fr}.quality-score,.quality-recommendations{padding:0 0 16px;border-right:0;border-left:0;border-bottom:1px solid var(--line)}.quality-recommendations{padding-top:16px;border-bottom:0}}
