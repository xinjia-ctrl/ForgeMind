...
export function renderReportHtml(model: ReportViewModel): string {
  ...
  const quality =
    model.quality === null
      ? emptyState("No run quality assessment recorded")
...
<body>
  <main>
    ...
    <section>
      <div class="section-heading"><div><p class="eyebrow">QUALITY ASSESSMENT</p><h2>Deterministic run quality</h2></div></div>
      ${quality}
    </section>
...
function renderQuality(quality: ReportQuality): string {
  const coverage =
    quality.coveragePercent === null ? "Unavailable" : `${quality.coveragePercent.toFixed(2)}%`;
  return `<article class="quality-panel ${cssToken(quality.verificationStrength)}">
    <div class="quality-score"><span>${escapeHtml(quality.outcome)}</span><strong>${escapeHtml(quality.verificationStrength)}</strong><small>verification</small></div>
    <dl class="quality-metrics">
      <div><dt>Evidence completeness</dt><dd>${quality.evidenceCompleteness.toFixed(2)}%</dd></div>
      <div><dt>Rework rounds</dt><dd>${formatNumber(quality.reworkRounds)}</dd></div>
      <div><dt>Policy violations</dt><dd>${formatNumber(quality.policyViolations)}</dd></div>
      <div><dt>Code coverage</dt><dd>${escapeHtml(coverage)}</dd></div>
    </dl>
    <div class="quality-recommendations"><strong>Confidence</strong><p>${(quality.confidence * 100).toFixed(1)}%</p></div>
  </article>`;
}
...
.status{padding:10px 14px;border:1px solid;border-radius:999px;font:800 12px ui-monospace,monospace;letter-spacing:.08em}.status.succeeded{color:var(--green);background:rgba(95,120,68,.1)}.status.failed,.status.blocked{color:var(--red);background:rgba(179,71,60,.09)}.status.running{color:var(--amber);background:rgba(168,111,44,.1)}.failure{display:flex;gap:18px;margin-top:28px;padding:24px;border:1px solid rgba(179,71,60,.42);background:linear-gradient(110deg,rgba(179,71,60,.09),transparent);border-radius:14px}.failure-mark{display:grid;place-items:center;flex:0 0 42px;height:42px;border-radius:50%;background:var(--red);color:#fffaf2;font-weight:900;font-size:24px}.failure h2{margin:0 0 8px}.failure p:last-child{margin:0;color:#7d302a;white-space:pre-wrap}
.overview{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;margin:28px 0 60px;background:var(--line);border:1px solid var(--line);border-radius:14px;overflow:hidden}.metric{background:var(--panel);padding:22px}.metric span{display:block;color:var(--muted);font-size:12px;margin-bottom:8px}.metric strong{font:700 25px ui-monospace,monospace}.section-heading{display:flex;align-items:flex-end;justify-content:space-between;margin:0 0 18px}.section-heading h2{font-size:26px;letter-spacing:-.025em;margin:0}
.stage-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:64px}.stage-card{background:linear-gradient(145deg,var(--panel2),var(--panel));border:1px solid var(--line);border-radius:12px;padding:18px}.stage-name{display:flex;align-items:center;justify-content:space-between;font:800 13px ui-monospace,monospace;letter-spacing:.08em}.stage-name i{width:7px;height:7px;border-radius:50%;background:var(--violet);box-shadow:0 0 12px rgba(132,107,122,.34)}dl{margin:20px 0 0}dl div{display:flex;justify-content:space-between;gap:14px;padding:8px 0;border-top:1px solid rgba(80,65,45,.09)}dt{color:var(--muted);font-size:12px}dd{margin:0;font:600 12px ui-monospace,monospace;text-align:right}
.player{display:flex;gap:8px}button{border:1px solid var(--line);background:var(--panel2);color:var(--text);border-radius:8px;padding:9px 13px;cursor:pointer}button:hover{border-color:#aa9476}button.primary{background:var(--cyan);border-color:var(--cyan);color:#fffaf2;font-weight:800}button:disabled{opacity:.35;cursor:not-allowed}.notice{border:1px solid rgba(168,111,44,.34);background:rgba(168,111,44,.08);color:#81551f;padding:12px 14px;border-radius:10px;margin-bottom:16px;font-size:13px}
.timeline{border-top:1px solid var(--line);margin-bottom:64px}.timeline-group{display:grid;grid-template-columns:130px 1fr;border-bottom:1px solid var(--line);padding:22px 0}.rail-label{position:sticky;top:20px;align-self:start;display:flex;flex-direction:column;gap:5px;font:800 13px ui-monospace,monospace;color:var(--cyan)}.rail-label small{color:var(--muted);font-weight:400}.event-list{display:flex;flex-direction:column;gap:10px}.timeline-item{display:grid;grid-template-columns:14px 1fr;gap:12px;padding:12px 14px 12px 6px;border:1px solid transparent;border-radius:10px;transition:.2s}.timeline-item.active{background:rgba(111,121,70,.07);border-color:rgba(111,121,70,.48);box-shadow:0 0 24px rgba(111,121,70,.08)}.event-dot{width:8px;height:8px;background:#a49179;border:2px solid var(--bg);outline:1px solid #a49179;border-radius:50%;margin-top:6px}.active .event-dot{background:var(--cyan);outline-color:var(--cyan);box-shadow:0 0 12px rgba(111,121,70,.38)}
.event-meta{display:flex;justify-content:space-between;gap:16px;color:var(--muted)}.event-meta code{color:#5d5143;font-size:12px}.event-meta time{font:11px ui-monospace,monospace}.event-content p{margin:7px 0 8px;line-height:1.5;white-space:pre-wrap}.badges{display:flex;flex-wrap:wrap;gap:6px}.badge{display:inline-block;border:1px solid #c9b9a4;border-radius:999px;color:#6f6252;padding:3px 7px;font:600 10px ui-monospace,monospace}.badge.stage{color:var(--cyan);border-color:rgba(111,121,70,.34)}.badge.outcome.passed,.badge.outcome.succeeded{color:var(--green);border-color:rgba(95,120,68,.34)}.badge.outcome.failed,.badge.outcome.rejected{color:var(--red);border-color:rgba(179,71,60,.34)}.badge.rework{color:var(--amber);border-color:rgba(168,111,44,.34)}details{margin-top:11px}details summary{cursor:pointer;color:var(--muted);font-size:12px}pre{max-height:360px;overflow:auto;background:#eee1cf;border:1px solid var(--line);border-radius:8px;padding:12px;color:#41382d;font:11px/1.55 ui-monospace,monospace;white-space:pre-wrap;overflow-wrap:anywhere}
.stage-empty,.empty{padding:20px;border:1px dashed var(--line);border-radius:10px;color:var(--muted);font-size:13px}.split{display:grid;grid-template-columns:1fr 1fr;gap:36px;margin-bottom:56px}.gate-list{display:flex;flex-direction:column;gap:10px}.gate{border:1px solid var(--line);border-left:3px solid;padding:16px;border-radius:10px;background:var(--panel)}.gate.passed{border-left-color:var(--green)}.gate.rejected{border-left-color:var(--red)}.gate strong{display:block;margin:12px 0 5px}.gate p{color:var(--muted);font-size:13px;line-height:1.5;margin:0;white-space:pre-wrap}.artifact-list{list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:8px}.artifact-list li{display:flex;gap:10px;padding:13px;border:1px solid var(--line);border-radius:10px;background:var(--panel)}.artifact-list strong{display:block;font:600 12px ui-monospace,monospace;overflow-wrap:anywhere}.artifact-list small{display:block;color:var(--muted);margin-top:5px}
.security-list{display:flex;flex-direction:column;gap:8px;margin-bottom:56px}.security-event{display:grid;grid-template-columns:88px 1fr auto;gap:16px;align-items:start;padding:14px;border:1px solid var(--line);border-left:3px solid var(--amber);border-radius:10px;background:var(--panel)}.security-event.approved,.security-event.allowed{border-left-color:var(--green)}.security-event.denied{border-left-color:var(--red)}.security-decision{font:800 11px ui-monospace,monospace;color:var(--amber)}.security-event.approved .security-decision,.security-event.allowed .security-decision{color:var(--green)}.security-event.denied .security-decision{color:var(--red)}.security-event strong{display:block;margin:8px 0 4px}.security-event p{color:var(--muted);font-size:12px;margin:0;overflow-wrap:anywhere}.security-event>time{color:var(--muted);font:10px ui-monospace,monospace}footer{display:grid;gap:8px;padding-top:24px;border-top:1px solid var(--line);color:var(--muted);font-size:11px}footer code{color:#5d5143;overflow-wrap:anywhere}
@media(max-width:760px){main{width:min(100% - 24px,1180px);padding-top:28px}.hero{display:block}.status{display:inline-block;margin-top:20px}.overview{grid-template-columns:1fr 1fr}.stage-grid{grid-template-columns:1fr}.timeline-heading{display:block}.player{margin-top:16px}.timeline-group{grid-template-columns:1fr;gap:16px}.rail-label{position:static}.event-meta{display:block}.event-meta time{display:block;margin-top:4px}.split{grid-template-columns:1fr}.requirement{font-size:15px}.security-event{grid-template-columns:1fr}.security-event>time{margin-top:4px}}
`;
